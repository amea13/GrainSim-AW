...
def update(*, grid, cfg_temp: Dict[str, Any], t: float) -> None:
    """
    ...
    if mode == "constant":
        grid.T[...] = np.full((Ny, Nx), T0, dtype=float)

    if mode == "constant_cooling":
        grid.T[...] = np.full((Ny, Nx), T0 - R * t, dtype=float)
    ...
    if mode == "linear_y":
        ...
        grid.T[...] = np.broadcast_to(T_line, (Ny, Nx)).copy()
